...
void visualizeQuadTree(QuadTreeNode* node, int depth) {
    ...
    if (node->figure != NULL) {
        printf("Leaf Node with shapes:\n");

        // Traverse and print the linked list of shapes in the leaf node
        Figure* currentShape = node->figure;
        while (currentShape != NULL) {
            printf("  Shape: %s, Type: %d\n", currentShape->name, currentShape->type);
