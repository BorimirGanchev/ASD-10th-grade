...
bool DFSRecc(Graph *graph, int vertex, bool *visited, int prev) {
    visited[vertex] = true;
    printf("%d ", vertex);

    for (int i = 0; i < graph->numVertices; i++) {
        if (graph->adjMatrix[vertex][i] && i != prev) {
            if (visited[i])
                return true;
                
            if (DFSRecc(graph, i, visited, vertex))
                return true;
        }
    }
    return false;
...
void DFS(Graph *graph, int startVertex) {
    ...
    bool hasCycle = DFSRecc(graph, startVertex, visited, startVertex);
    free(visited);
